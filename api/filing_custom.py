...
import requests #Moving request into here
...
class Filing(object):
    ...
    def __init__(self, object_id, filepath=None, URL=None, json=None):
        ...
        self.response = None #Adding property
        ...
        if json:
            ...
            #else:
                #Not Needed, Will not be saving locally
                #self.filepath = get_local_path(self.object_id)

                #Not Needed, This looks for xml file locally
                '''if URL:
                    self.URL = URL
                else:
    ...
    def _set_dict_from_response(self): #New function for response to dict


            try:
                self.raw_irs_dict = xmltodict.parse(self.response)
            except Exception as e:
                #self.reconnect()
                print( 'ERROR: Some problem while trying to call xmltodict\n%s' % e)
            ...
            try:
                ...
                #self.reconnect()
                print( 'ERROR: Some problem while trying to call raw_irs_dict\n%s' % e)
